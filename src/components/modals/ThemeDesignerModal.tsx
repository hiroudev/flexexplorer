...
export default function ThemeDesignerModal({
  ...
  useEffect(() => {
    ...
    const instance = mountThemeDesigner(hostRef.current, {
      initial,
      onPreview: (theme: FlexTheme) => { applyTheme(theme, document.documentElement) },
      onSave: (result: ThemeDesignerSaveResult) => onSave(result),
      onCancel: () => { applyTheme(restoreThemeKey, document.documentElement); onClose() },
    })
  ...
  return (
    <>
      <div
        onMouseDown={() => { applyTheme(restoreThemeKey, document.documentElement); onClose() }}
        style={{ position: 'fixed', inset: 0, zIndex: 120, background: 'rgba(0,0,0,.4)' }}
      />
      <div style={{
        position: 'fixed', left: '50%', top: '50%', transform: 'translate(-50%,-50%)',
        zIndex: 121, width: 600, maxHeight: '86vh', overflowY: 'auto',
        background: 'var(--bg-panel)', border: '1px solid var(--border)', borderRadius: 12,
        boxShadow: '0 24px 70px var(--shadow)', padding: 18, fontFamily: 'var(--font)',
      }}>
